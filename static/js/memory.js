...
players.forEach((player, i) => {
  ...
  if (player.trim() !== "") {
    // a note for myself:    in JS it's !== and not !=
...
function setPlayerAsActive(playerIndex) {
  document.querySelectorAll(".player-card").forEach((card, i) => {
    if (i === playerIndex) card.classList.add("active");
...
function handleSelectingCards(card) {
  if (lockBoard) return;
  if (isFlipped || card === firstCard || card.classList.contains("matched"))
    return; //see "handleMatch" for "matched"
...
function gameOver() {
  ...
  setTimeout(() => {
    ...
    if (winners.includes(",")) {
      ...
      winnerName.textContent = `${winners} WON THE GAME!`;
    }
